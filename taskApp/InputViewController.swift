...
class InputViewController: UIViewController {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        try! realm.write {
            ...
            self.task.category = self.category1.text!
    ...
    func setNotification(task: Task) {
        ...
        //タイトルと内容の設定（中身がない場合メッセージ無しで音だけの通知となるので「（xx無し）」を表示する
        if task.title == "" {
        ...
        //identifier, content, trigger からローカル通知を作成（identifierが同じだとローカル通知を上書き保存
        let request = UNNotificationRequest(identifier: String(task.id), content: content, trigger: trigger)
        //ローカル通知を登録
        let center = UNUserNotificationCenter.current()
        center.add(request) { (error) in
            print(error ?? "ローカル通知登録　OK") //errorがnilならローカル通知の登録に成功したと表示
        }
        //未通知のローカル通知一覧をログ出力
